...
import { ChevronLeft, Handshake, ArrowRightLeft, Shield, Share2, Heart, X } from 'lucide-react';
...
export default function DiplomacyPanel() {
  ...
  const createAgreement = (type) => {
    ...
    updateGameState(prev => {
      ...
      const agreement = {
        type,
        between: [prev.currentPlayerIndex, targetPlayer],
  ...
  const diplomacyActions = [
    ...
    { key: 'alliance', label: t.diplomacy.formAlliance, icon: Handshake },
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col">
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        <div className="max-w-2xl mx-auto space-y-6">
              ...
              {otherPlayers.map(p => (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs">{p.playerName} · Pop: {p.resources.population}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {targetPlayer !== null && (
            <div>
              <h3 className="text-gray-400 text-xs font-heading uppercase tracking-wider mb-3">
                {t.general.availableActions}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {diplomacyActions.map(da => (
                  <button
                    key={da.key}
                    onClick={() => createAgreement(da.key)}
                    ...
                    <div key={i} className="flex items-center gap-3 bg-gray-900/60 rounded p-3 border border-gray-800">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-500 text-xs">
                          {other?.countryName} · {t.general.round} {ag.round}
                        </p>
                      </div>
