      ...
      <div className="max-w-md w-full bg-gray-900 border border-gray-700 rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-2">
            <button
              onClick={() => setTab('save')}
              className={`px-3 py-1 rounded text-sm ${tab === 'save' ? 'bg-orange-600 text-white' : 'text-gray-400 hover:text-white'}`}
            >
              {t.save.saveGame}
            </button>
            <button
              onClick={() => setTab('load')}
              className={`px-3 py-1 rounded text-sm ${tab === 'load' ? 'bg-orange-600 text-white' : 'text-gray-400 hover:text-white'}`}
            >
              {t.save.loadGame}
            </button>
          </div>
          ...
          <div>
            <div className="flex gap-2 mb-4">
              ...
              <button
                ...
                className="px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white rounded text-sm font-medium disabled:opacity-40 flex items-center gap-1"
              >
                  ...
                  <div key={key} className="flex items-center justify-between bg-gray-800/60 rounded p-3">
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">
                        {t.general.round} {saves[key].currentRound} · {saves[key].players?.length || 0} players
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => { loadGame(key); onClose(); }}
                        className="px-3 py-1 bg-orange-600/30 text-orange-300 rounded text-xs hover:bg-orange-600/50"
                      >
                        {t.save.load}
                      </button>
                      <button
                        onClick={() => deleteSave(key)}
