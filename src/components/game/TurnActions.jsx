...
import { Telescope, MapPin, Hammer, FlaskConical, SkipForward } from 'lucide-react';

const ACTIONS = [
  { key: 'explore', emoji: '🔭', icon: Telescope, apCost: 1, descKey: 'explore' },
...
export default function TurnActions({ onAction, actionMode }) {
  ...
  return (
    <div className="p-3 flex flex-col gap-2">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-gray-500">
                {action.apCost} AP{action.extraCost ? ` + ${action.extraCost}` : ''}
              </p>
            </div>
      ...
      <button
        onClick={endTurn}
        className="w-full mt-2 py-2.5 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-heading font-semibold text-sm transition-colors flex items-center justify-center gap-2"
      >
        <SkipForward size={15} />
        {t.actions.endTurn}
      </button>
