      ...
      <div className="p-3 border-b border-gray-700/50">
        <div className="flex items-center justify-between text-xs text-gray-400 mb-1.5">
          ...
          <span className="font-mono">
            {t.actions.actionPoints}: <span className="text-orange-400 font-bold">{gameState.actionPoints}</span>
          </span>
        </div>
        <div className="w-full bg-gray-800 rounded-full h-1.5">
          <div
            className="bg-orange-500 rounded-full h-1.5 transition-all"
            style={{ width: `${Math.min(100, (gameState.currentRound / gameState.settings.gameLength) * 100)}%` }}
          />
        </div>
        {/* AP dots */}
        <div className="flex gap-1 mt-2">
          {Array.from({ length: gameState.settings.actionPointsPerTurn || 3 }).map((_, i) => (
            <div
              key={i}
              className={`w-4 h-4 rounded-full border-2 ${i < gameState.actionPoints ? 'bg-orange-500 border-orange-400' : 'bg-gray-800 border-gray-600'}`}
            />
          ))}
        </div>
      </div>
      ...
      <div className="px-3 py-2 border-b border-gray-700/50">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className={`text-xs font-bold ${
            (player.scores?.livingConditions || 0) > 60 ? 'text-green-400' :
            (player.scores?.livingConditions || 0) > 30 ? 'text-yellow-400' : 'text-red-400'
          }`}>{player.scores?.livingConditions || 0}</span>
        </div>
        <div className="w-full bg-gray-800 rounded-full h-1.5">
          <div
